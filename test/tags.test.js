...
describe('Noteful API - Tags', function () {
  ...
  describe('GET /api/tags/:id', function () {
    ...
    it('should respond with a 401 for an id that does not exist', function () {
    ...
    it('should create and return a new item when provided valid data', function () {
      ...
      return chai.request(app)
        ...
        .then(function (res) {
          ...
          return Tag.findById({ userId: user.id, _id: body.id});
        })
        .then(data => {
  ...
  describe('PUT /api/tags/:id', function () {
    ...
    it('should respond with a 400 for an invalid id', function () {
      ...
      return chai.request(app)
        .put('/api/tags/NOT-A-VALID-ID').set('Authorization', `Bearer ${token}`)
        .send(updateItem)
        .then(res => {
          expect(res).to.have.status(400);
          expect(res.body.message).to.equal('The `id` is not valid');
    ...
    it('should respond with a 404 for an id that does not exist', function () {
      ...
      // The string "DOESNOTEXIST" is 12 bytes which is a valid Mongo ObjectId
      return chai.request(app)
        .put('/api/tags/DOESNOTEXIST').set('Authorization', `Bearer ${token}`)
        .send(updateItem)
        .then(res => {
    ...
    it('should return an error when given a duplicate name', function () {
      return Tag.find({ userId: user.id }).limit(2)
        .then(results => {
          ...
          return chai.request(app)
            .put(`/api/tags/${item1.id}`).set('Authorization', `Bearer ${token}`)
            .send(item1);
        })
        .then(res => {
          expect(res).to.have.status(400);
  ...
  describe('DELETE /api/tags/:id', function () {
    ...
    it('should respond with a 400 for an invalid id', function () {
      return chai.request(app)
        .delete('/api/tags/NOT-A-VALID-ID').set('Authorization', `Bearer ${token}`)
        .then(res => {
          expect(res).to.have.status(400);
          expect(res.body.message).to.equal('The `id` is not valid');
        });
