...
describe('Noteful API - Notes', function () {
    ...
    it('should create and return a new item when provided valid data', function () {
      ...
      return chai.request(app)
        ...
        .then(function (_res) {
          ...
          return Note.findById({ userId: user.id, _id: res.body.id });
        })
        .then(data => {
          expect(res.body.id).to.equal(data.id);
          expect(res.body.title).to.equal(data.title);
  ...
  describe('PUT /api/notes/:id', function () {
    ...
    it('should update the note when provided valid data', function () {
      ...
      return Note.findOne({ userId: user.id })
        ...
        .then(function (res) {
          ...
          expect(res).to.be.json;
          expect(res.body).to.be.a('object');
          expect(res.body).to.have.all.keys('id', 'title', 'content', 'createdAt', 'userId', 'updatedAt', 'folderId', 'tags');
          expect(res.body.id).to.equal(data.id);
    ...
    it('should respond with status 400 and an error message when `id` is not valid', function () {
      ...
      return chai.request(app)
        .put('/api/notes/NOT-A-VALID-ID').set('Authorization', `Bearer ${token}`)
        .send(updateItem)
        .then(res => {
          expect(res).to.have.status(400);
          expect(res.body.message).to.equal('The `id` is not valid');
